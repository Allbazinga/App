...
import com.cwl.app.task.ImageCache;
import com.cwl.app.task.LoadImageTask;
import com.cwl.app.task.LoadVideoImageTask;
import com.cwl.app.ui.AlertDialog;
import com.cwl.app.ui.ChatActivity;
import com.cwl.app.ui.ContextMenu;
import com.cwl.app.ui.ShowBigImage;
import com.cwl.app.ui.ShowNormalFileActivity;
import com.cwl.app.ui.ShowVideoActivity;
import com.cwl.app.ui.UserInfoActivity;
import com.cwl.app.utils.CommonUtils;
import com.cwl.app.utils.Constants;
import com.cwl.app.utils.ImageOptions;
import com.cwl.app.utils.SmileUtils;
...
public class MessageAdapter extends BaseAdapter {
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			 ...
			 * if(attr.equals("card")){//判断扩展属性是否是名片 return message.direct ==
			 * EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_FILE :
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			if (message.getType() == EMMessage.Type.IMAGE) {
				...
				try {
					holder.head_iv = (ImageView) convertView
							.findViewById(R.id.iv_userhead);
					holder.file_pic = (ImageView) convertView
							.findViewById(R.id.file_pic);
		...
		if (message.direct == EMMessage.Direct.SEND) {
			 ...
			 * (intent,ChatActivity.REQUEST_CODE_ADD_TO_BLACKLIST); return true;
			 * } });
			 */
		...
		if (message.direct == EMMessage.Direct.RECEIVE
				&& chatType != ChatType.GroupChat) {// 非群聊显示对方头像
			if (message.getFrom().equals(Constants.KEFU)) {
				...
				/*** 获取本地数据库用户信息end ***/
				// ImageLoader.getInstance().displayImage("http://tp2.sinaimg.cn/1193258161/50/22829537315/1",
				// holder.head_iv, ImageOptions.getOptions());
				// ImageLoader.getInstance().displayImage(user.getHeaderurl(),
				// holder.head_iv, ImageOptions.getOptions());
			}
		} else if (message.direct == EMMessage.Direct.RECEIVE
				&& chatType == ChatType.GroupChat) {// 群聊的时候显示用户头像
			// 暂时不考虑群聊ImageLoader.getInstance().displayImage(headurl,
			// holder.head_iv, ImageOptions.getOptions());

		} else {// 所有自己发送的图片显示自己的头像
				// ImageLoader.getInstance().displayImage(PreferenceUtils.getInstance(context).getSettingUserPic(),
				// holder.head_iv, ImageOptions.getOptions());
		}
		...
		if (message.direct == EMMessage.Direct.RECEIVE) {
			...
			if (message.status == EMMessage.Status.INPROGRESS) {
				...
				if (imgBody.getLocalUrl() != null) {
					...
					//String filePath = com.cwl.app.utils.ImageUtils
					//		.getImagePath(remotePath);
					String thumbRemoteUrl = imgBody.getThumbnailUrl();
					String thumbnailPath = com.cwl.app.utils.ImageUtils
		...
		if (!CommonUtils.isNullOrEmpty(attr)) {// 判断扩展属性是否为空
			if (attr.equals("card")) {// 判断扩展属性是否是名片
				...
				ImageLoader.getInstance().displayImage(
						fileMessageBody.getRemoteUrl(), holder.file_pic,
						ImageOptions.get_gushi_Options());
				holder.ll_container.setOnClickListener(new OnClickListener() {

					@Override
					public void onClick(View view) {
						String user = message.getStringAttribute("user", null);// 获取扩展属性
						Toast.makeText(activity, "在MessageAdapter.handleFileMessage中修改", Toast.LENGTH_SHORT).show();
						/*if (!CommonUtils.isNullOrEmpty(user)) {
							context.startActivity(new Intent(context,
									Userinfo.class).putExtra("userId", user));
						}*/
					}
				});
			}
			...
			holder.tv_file_size.setText(TextFormater
					.getDataSize(fileMessageBody.getFileSize()));// 文件大小
			...
			holder.ll_container.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View view) {
					...
					if (file != null && file.exists()) {
						...
						// 下载
						context.startActivity(new Intent(context,
								ShowNormalFileActivity.class).putExtra(
								"msgbody", fileMessageBody));
					}
					if (message.direct == EMMessage.Direct.RECEIVE
							&& !message.isAcked) {
						try {
	...
	public void add() {
		/*
		 * new AlertDialog.Builder(context) .setTitle("请输入您的昵称")
		 * .setIcon(android.R.drawable.ic_dialog_info) .setView("加好友")
		 * .setPositiveButton("确定", new DialogInterface.OnClickListener() {
		 * public void onClick(DialogInterface dialog, int which) { String
		 * nickname ="w ye buzhidao "; RequestParams params = new
		 * RequestParams();
		 * 
		 * 
		 * dialog.dismiss(); //设置你的操作事项 } }) .setNegativeButton("取消", null)
		 * .show();
		 */
	}
	...
	class MapClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			activity.startActivity(intent);*/
			Toast.makeText(activity, "在MessageAdapter.MapClickListener中修改", Toast.LENGTH_SHORT).show();
		}
